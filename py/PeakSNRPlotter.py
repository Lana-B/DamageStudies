...
colors = iter(cm.gist_rainbow(np.linspace(0, 1,5)))
...
for j,file_iter in enumerate(files):
    ...
    if(noisehist):
        ...
        histVals=ax1.hist(input_data.ravel(),bins=60,range=(0,60),histtype='step',label=file_iter[:-4],color=colorChoice,linewidth=2)
        ...
        print(histVals[0][15:].max(),histVals[0][15:].argmax())
        peak_max=histVals[0][15:].argmax()+15
        print('max',peak_max)
        try:
            popt2,pcov2=curve_fit(gaus,mphist[peak_max-2:peak_max+10],histVals[0][peak_max-2:peak_max+10],p0=[5000,21,8])
        except:
            popt2=[1,1,1]
        print(popt2)
        ax1.plot(mphist,gaus(mphist,*popt2),color=colorChoice,linestyle="dotted",label=file_iter[:-4]+"fit")
        noise_std=np.append(noise_std,popt2[1])
        dataset_label=np.append(dataset_label,file_iter[:-4])
    # else:
...
ax1.set_xlabel('Signal/Noise', fontsize=18)
